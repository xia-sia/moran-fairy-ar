/**
 * 사월의 부귀, 개화 — AR 코어 v4
 * 권한 팝업 → iOS 자이로 요청 → MindAR 직접 시작 (카메라는 MindAR이 처리)
 */
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('btn-allow').addEventListener('click', onAllowClick);
});
...
async function onAllowClick() {
  ...
  // iOS 13+: DeviceOrientation 권한은 유저 제스처 안에서 요청해야 함
  if (
  ...
  // MindAR 시작 (카메라 권한 팝업은 MindAR이 자체적으로 띄움)
  await initAR();
}
...
async function initAR() {
  try {
    ...
    const name = err?.name || '';
    if (name === 'NotAllowedError')   msg = '카메라 권한이 거부됐어요.\n브라우저 설정에서 카메라를 허용해주세요.';
    if (name === 'NotFoundError')     msg = '카메라를 찾을 수 없어요.';
    if (name === 'NotSupportedError') msg = 'Safari 또는 Chrome을 사용해주세요.';
    if (name === 'AbortError')        msg = '카메라 시작이 중단됐어요. 다시 시도해주세요.';
    showError(msg);
  }
}
...
function createFairy(cfg) {
  ...
  const glow = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({
      color: cfg.color, transparent: true, opacity: 0.18, side: THREE.BackSide
    })
  );
  group.add(glow);
  ...
  const petals = new THREE.Points(geo,
    new THREE.PointsMaterial({
      color: cfg.color, size: 0.005, transparent: true, opacity: 0.7, sizeAttenuation: true
    })
  );
  group.add(petals);
...
function attachGyro() {
  window.addEventListener('deviceorientation', (e) => {
    ...
    deviceQuat.setFromEuler(new THREE.Euler(
...
function spawnPetalBurst(position, color, count) {
  ...
  const mat = new THREE.PointsMaterial({
    color, size: 0.012, transparent: true, opacity: 1, sizeAttenuation: true
  });
...
function updateHints() {
  ...
  fairyMeshes.forEach((fairy, i) => {
    ...
    if (pos.x > -1 && pos.x < 1 && pos.y > -1 && pos.y < 1) {
      ...
      hint.querySelector('.hint-arrow').style.transform =
        `rotate(${THREE.MathUtils.radToDeg(angle) + 45}deg)`;
      hint.querySelector('.hint-dist').textContent =
        fairy.position.distanceTo(camera.position).toFixed(1) + 'm';
    }
  });
}
